...
import { upsertCategoryAction, deleteCategoryAction, upsertMenuItemAction, deleteMenuItemAction, createSignedImageUploadUrlAction } from "@/app/(admin)/admin/catalog/actions";
...
export const CatalogManager = ({ categories, menuItems }: CatalogManagerProps) => {
  const [categoryDialog, setCategoryDialog] = useState<CategoryDialogState>({ open: false });
  ...
  const onSubmitItem = itemForm.handleSubmit(async (values) => {
    ...
    try {
      const fileList = (document.getElementById("menu-item-image") as HTMLInputElement | null)?.files;
      ...
      .map((category) => (
        <Tr key={category.id}>
          ...
          <Td className="flex gap-2">
            <Button size="sm" variant="outline" onClick={() => handleOpenCategoryDialog(category)}>
              <Pencil className="mr-1 h-4 w-4" /> Edit
            </Button>
            <Button
              size="sm"
              variant="destructive"
              onClick={() => startTransition(async () => deleteCategoryAction(category.id))}
            >
    ...
    return menuItems
      ...
      .map((item) => (
        <Tr key={item.id}>
          ...
          <Td className="flex gap-2">
            <Button size="sm" variant="outline" onClick={() => handleOpenItemDialog(item)}>
              <Pencil className="mr-1 h-4 w-4" /> Edit
            </Button>
            <Button
              size="sm"
              variant="destructive"
              onClick={() => startTransition(async () => deleteMenuItemAction(item.id))}
            >
              <Trash2 className="mr-1 h-4 w-4" /> Hapus
            </Button>
          </Td>
  ...
  return (
    <div className="space-y-6">
        ...
        <TabsContent value="items">
          <Card>
            <CardHeader className="flex items-center justify-between gap-2">
              ...
              <Button onClick={() => handleOpenItemDialog()} disabled={categories.length === 0}>
        ...
        <DialogContent>
          ...
          <form className="space-y-4" onSubmit={onSubmitCategory}>
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Contoh: Makanan" {...categoryForm.register("name")} />
            ...
            <div className="space-y-2">
              ...
              <Input type="number" {...categoryForm.register("position", { valueAsNumber: true })} />
            ...
            <div className="flex items-center justify-between rounded-md border border-border p-3">
              ...
              <Switch
                ...
                onCheckedChange={(checked) => categoryForm.setValue("is_active", checked)}
              />
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setCategoryDialog({ open: false })}>
                Batal
              </Button>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{itemDialog.item ? "Edit Menu" : "Tambah Menu"}</DialogTitle>
          </DialogHeader>
          <form className="space-y-4" onSubmit={onSubmitItem}>
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Contoh: Nasi Goreng" {...itemForm.register("name")} />
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input type="number" step="100" {...itemForm.register("price", { valueAsNumber: true })} />
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Posisi</label>
                <Input type="number" {...itemForm.register("position", { valueAsNumber: true })} />
            ...
            <div className="flex items-center justify-between rounded-md border border-border p-3">
              ...
              <Switch
                ...
                onCheckedChange={(checked) => itemForm.setValue("is_available", checked)}
              />
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setItemDialog({ open: false })}>
                Batal
              </Button>
              <Button type="submit" disabled={isPending || uploading}>
                {isPending || uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Simpan"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
