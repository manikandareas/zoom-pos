...
import { ORDER_STATUS_COLORS, ORDER_STATUS_LABEL, ORDER_STATUSES, CURRENCY_FORMATTER } from "@/lib/constants";
import type { AdminOrderRow } from "@/lib/data/orders";
import type { Room } from "@/lib/supabase/types";
import { acceptOrderAction, rejectOrderAction, updateOrderStatusAction, markAsBilledAction } from "@/app/(admin)/admin/orders/actions";
...
const fetchOrderDetails = async (client: SupabaseClient, orderId: string): Promise<AdminOrderRow | null> => {
...
export const OrdersBoard = ({ orders: initialOrders, rooms }: OrdersBoardProps) => {
  ...
  const [rejectDialog, setRejectDialog] = useState<{ open: boolean; orderId?: string }>({ open: false });
    ...
    return orders.filter((order) => {
      ...
      const matchesSearch = searchFilter
        ? order.note?.toLowerCase().includes(searchFilter.toLowerCase()) ||
          order.rejection_reason?.toLowerCase().includes(searchFilter.toLowerCase()) ||
          order.items.some((item) =>
            item.menu_item_name.toLowerCase().includes(searchFilter.toLowerCase())
          )
        : true;
  ...
  const handleNextStatus = (orderId: string, status: "IN_PREP" | "READY" | "DELIVERED") => {
    setActionTarget(orderId);
    startTransition(async () => {
  ...
  const isActionPending = (orderId: string) => isPending && actionTarget === orderId;
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-4">
          ...
          <div className="space-y-2">
            <span className="text-xs font-medium text-muted-foreground">Kamar</span>
                  ...
                  <Tr key={order.id}>
                    ...
                    <Td>
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground">{order.room.label}</span>
                      </div>
                    </Td>
                    ...
                    <Td>
                      <Badge className={cn("capitalize", ORDER_STATUS_COLORS[order.status])}>
                    ...
                    <Td className="space-y-2 text-xs">
                      {order.status === "PENDING" && (
                        <div className="flex flex-wrap gap-2">
                          ...
                          <Dialog
                            open={rejectDialog.open && rejectDialog.orderId === order.id}
                            onOpenChange={(open) => {
                              if (open) {
                                setRejectDialog({ open: true, orderId: order.id });
                                setRejectReason("");
                              } else {
                                setRejectDialog({ open: false });
                                setRejectReason("");
                            ...
                            <DialogContent>
                              ...
                              <DialogFooter>
                                ...
                                <Button
                                  type="button"
                                  variant="destructive"
                                  disabled={!rejectReason.trim() || isActionPending(order.id)}
                      ...
                      {nextStatus && (
                        ...
                        >
                          {isActionPending(order.id) ? (
                            ...
                            <>Ke {ORDER_STATUS_LABEL[nextStatus]}</>
                          )}
                        </Button>
                      )}
